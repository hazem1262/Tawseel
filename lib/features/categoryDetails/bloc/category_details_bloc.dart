...
class CategoryDetailsBlocEvent with _$CategoryDetailsBlocEvent {
  ...
  const factory CategoryDetailsBlocEvent.getSubCategories(int categoryId) = GetCategoryDetailsSubCategories;

  const factory CategoryDetailsBlocEvent.selectSubCategories(int categoryId) = SelectCategoryDetailsSubCategory;

  const factory CategoryDetailsBlocEvent.getMarketPlaces(List<int>? category_ids) = GetCategoryDetailsMarketPlaces;

  const factory CategoryDetailsBlocEvent.addMarketPlaceToFavorite(int id) = AddMarketPlaceToFavorite;

  const factory CategoryDetailsBlocEvent.removeMarketPlaceToFavorite(int id) = RemoveMarketPlaceFromFavorite;

  const factory CategoryDetailsBlocEvent.reset() = ResetCategoryDetailsRefreshData;
}
    ...
    on<CategoryDetailsBlocEvent>((event, emit) async {
      if (event is GetCategoryDetailsSubCategories) {
        ...
        try {
          ...
          emit(state.copyWith(categoriesIsLoading: false, error: "", categories: cats.data));
        } catch (e) {
      ...
      if (event is SelectCategoryDetailsSubCategory) {
        ...
        try {
          ...
          emit(state.copyWith(categoriesIsLoading: false, error: "", categories: cats));
        } catch (e) {
          emit(state.copyWith(categoriesIsLoading: false, error: e.toString()));
          debugPrint('Exception : $e');
        }
